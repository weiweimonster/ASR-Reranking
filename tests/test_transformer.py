...
def test_log_probabilities():
    # Load model
    model = CharacterLevelTransformer(2, 64, 4, 128, 0.1, 36)
    state_dict = torch.load('unit_test_transformer_data/unit_test_state_dict.pth')
    model.load_state_dict(state_dict)
    model.eval()

    # Compute log-probabilities in base 2 (to be used in perplexity)
    input_tokens, target_tokens = torch.load('unit_test_transformer_data/testing_data.pth')
    log_probabilities = model.log_probability(input_tokens, target_tokens, base=2)
    target_log_probabilities = torch.load('unit_test_transformer_data/target_log_probs.pth')
